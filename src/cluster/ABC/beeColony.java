...
public  class beeColony {
	...
	int limit = 25;  /*A food source which could not be improved through "limit" trials is abandoned by its employed bee*/
	...
	double[] lb;// = new double[D];//-5.12; /*lower bound of the parameters. */
	double[] ub;// = new double[D];//5.12; /*upper bound of the parameters. lb and ub can be defined as arrays for the problems of which parameters have different bounds*/
...
//	FunctionCallback function = &sphere;
	...
	/*set fitfun*/
	public void SetFitFunc(TargetFun targetfun){
		myTargetFunc=targetfun;
	}
	public TargetFun getFitFunc(){
		return myTargetFunc;
	}
	
	public void InitParam(int inputD, TargetFun targetfun, double[] inputub, double[] inputlb){
		D=inputD;
		lb = new double[D];
		ub = new double[D];
		...
		SetFitFunc(targetfun);
		SetBoundary(inputub,inputlb);
	}
	
	/*Set ub and lb*/
	void SetBoundary(double[] ubp,double[] lbp){
		if(ubp.length==1){
			for(int i=0;i<D;i++){
				ub[i]=ubp[0];
			}
	  ...
	  /*Employed Bee Phase*/
	   for (i=0;i<FoodNumber;i++)
	       ...
	       // }
	        ...
	        //vec change
//	        for(int param2change=0;param2change<solution.length;param2change++){
...
{
	return myTargetFunc.getfitnessvalue(sol);
